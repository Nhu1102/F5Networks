...
class TestManagerSSLOAbove60(unittest.TestCase):
    def setUp(self):
...
class TestManagerSSLOBelow60(unittest.TestCase):
    def setUp(self):
        self.spec = ArgumentSpec()
        self.p1 = patch('time.sleep')
        self.p1.start()
    ...
    def test_resolver_object_correct_json_generated_for_v_50(self, *args):
        # Configure the arguments that would be sent to the Ansible module
        expected = load_fixture('sslo_resolver_create_ver_5_0_generated.json')
        set_module_args(dict(
            fwd_zones=[
                dict(
                    zone='foobar',
