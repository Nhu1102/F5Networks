    ...
    def update_json(self, operation):
        ...
        if operation == "CREATE":
            ...
            try:
                ...
                self.config["inputProperties"][0]["value"][
                    "deploymentReference"] = "https://localhost/mgmt/shared/iapp/blocks/" + id
            ...
            if operation in ["MODIFY"] and self.want.snat == "snatlist":
                ...
                for key in response['contents']['items'][0]['inputProperties']:
            ...
            if operation in ["MODIFY"]:
                ...
                if self.want.ipFamily == "ipv4":
                    ...
                    if self.ssloVersion >= 9.0:
                        uri = "/mgmt/tm/net/self/~Common~ssloN_{0}-70-0-flt-S4ForFltIP.app~ssloN_{0}-70-0-flt-S4".format(self.local_name)
                    else:
                        uri = "/mgmt/tm/net/self/~Common~{0}.app~{0}-70-0-flt-S4".format(self.want.name)
                    ...
                    resp_in = response['contents']["address"]
                    ...
                    ## =================================
                    if self.ssloVersion >= 9.0:
                        uri = "/mgmt/tm/net/self/~Common~ssloN_{0}-70-0-flt-D4ForFltIP.app~ssloN_{0}-70-0-flt-D4".format(self.local_name)
                    else:
                        uri = "/mgmt/tm/net/self/~Common~{0}.app~{0}-70-0-flt-D4".format(self.want.name)
                    query = "?$select=address"
                    response = self.client.get(uri + query)
                    ...
                    resp_out = response['contents']["address"]
                    ...
                    ## =================================
                    if self.ssloVersion >= 9.0:
                        uri = "/mgmt/tm/net/self/~Common~ssloN_{0}-70-0-flt-S6ForFltIP.app~ssloN_{0}-70-0-flt-S6".format(self.local_name)
                    else:
                        uri = "/mgmt/tm/net/self/~Common~{0}.app~{0}-70-0-flt-S6".format(self.want.name)
                    query = "?$select=address"
                    response = self.client.get(uri + query)
                    ...
                    ## =================================
                    if self.ssloVersion >= 9.0:
                        uri = "/mgmt/tm/net/self/~Common~ssloN_{0}-70-0-flt-D6ForFltIP.app~ssloN_{0}-70-0-flt-D6".format(self.local_name)
                    else:
                        uri = "/mgmt/tm/net/self/~Common~{0}.app~{0}-70-0-flt-D6".format(self.want.name)
                    query = "?$select=address"
                    response = self.client.get(uri + query)
                ...
                if self.want.device_to_vlan == None:
                    ...
                    if response['contents']["items"] != []:
                        self.config["inputProperties"][1]["value"][self.index]["existingBlockId"] = response['contents']["items"][0]["id"]
                        self.config["inputProperties"][2]["value"]["customService"]["connectionInformation"]["fromBigipNetwork"]["networkBlockId"] = response['contents']["items"][0]["id"]
                ...
                if self.want.device_from_vlan == None:
                    ...
                    if response['contents']["items"] != []:
                        self.config["inputProperties"][1]["value"][self.index]["existingBlockId"] = response['contents']["items"][0]["id"]
                        self.config["inputProperties"][2]["value"]["customService"]["connectionInformation"]["toBigipNetwork"]["networkBlockId"] = response['contents']["items"][0]["id"]
